@echo off
REM WindMusic offline installer for Windows.
...
REM   - windmusic.crt
REM   - windmusic-<version>.msix     (any .msix matching windmusic-*.msix)
REM
...
echo.
echo === WindMusic offline installer ===
echo.

REM --- 2. Install cert to Trusted People (idempotent via certutil) -----------
if not exist "%~dp0simpmusic.crt" (
    echo [ERROR] windmusic.crt not found next to install.bat
    pause
    exit /b 1
)

echo [1/3] Importing windmusic.crt to LocalMachine\TrustedPeople ...
...
if not defined MSIX (
    echo [ERROR] No windmusic-*.msix found in this folder.
    pause
    exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ForceUpdateFromAnyVersion -ErrorAction Stop; exit 0 } catch { Write-Host 'In-place update failed; removing existing WindMusic and reinstalling...'; Get-AppxPackage -Name 'Simpmusic' | Remove-AppxPackage -ErrorAction SilentlyContinue; try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ErrorAction Stop; exit 0 } catch { Write-Host $_; exit 1 } }"
...
REM --- 4. Launch app ---------------------------------------------------------
echo.
echo [3/3] Launching WindMusic ...
start "" "shell:appsFolder\Simpmusic_ejp2bhxmz1qq6!Simpmusic"

echo.
echo WindMusic installed successfully.
echo To uninstall: Settings -^> Apps -^> Installed apps -^> WindMusic -^> Uninstall
echo.
pause
endlocal
