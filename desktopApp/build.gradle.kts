...
// desktopApp — JVM application module for WindMusic Desktop.
//
...
// Append WindMusic-specific keys to Conveyor's generated config file and
...
tasks.named<hydraulic.conveyor.gradle.WriteConveyorConfigTask>("writeConveyorConfig") {
    ...
    doLast {
        destination.get().asFile.appendText(
            """
            |app.fsname = windmusic
            |app.display-name = WindMusic
            |app.rdns-name = com.maxrave.windmusic
            |
...
compose.desktop {
    application {
        mainClass = "com.maxrave.windmusic.MainKt"
                ...
                val macExtraPlistKeys =
                    ...
                    <array>
                        <dict>
                            ...
                            <string>com.maxrave.windmusic.deeplink</string>
                            <key>CFBundleURLSchemes</key>
                            <array>
                                <string>windmusic</string>
                            </array>
                        </dict>
                    </array>
                    """.trimIndent()
...
tasks.register("packageConveyorAppImage") {
    ...
    doLast {
        ...
        val iconDst = appDir.resolve("windmusic.png")
        if (!iconDst.exists() && iconSrc.exists()) {
            FileUtils.copyFile(iconSrc, iconDst)
        }

        val versionName = libs.versions.version.name.get()
        val desktopFile = appDir.resolve("windmusic.desktop")
        desktopFile.writeText(
            """[Desktop Entry]
            |Type=Application
            |Version=1.0
            |Name=WindMusic
            |Comment=WindMusic v$versionName - FOSS YouTube Music Client
            |Exec=bin/windmusic %u
            |Icon=windmusic
            |Terminal=false
            |Categories=Audio;AudioVideo;
            |StartupWMClass=com-maxrave-windmusic-MainKt
            |MimeType=x-scheme-handler/windmusic;
            |
            """.trimMargin(),
        )
        ...
        appRun.writeText(
            ...
            |if [ ! -f "${'$'}ICON_DIR/windmusic.png" ] || [ "${'$'}HERE/windmusic.png" -nt "${'$'}ICON_DIR/windmusic.png" ]; then
            |    mkdir -p "${'$'}ICON_DIR"
            |    cp "${'$'}HERE/windmusic.png" "${'$'}ICON_DIR/windmusic.png"
            ...
            |sed "s|Exec=bin/windmusic|Exec=${'$'}APPIMAGE_PATH|" "${'$'}HERE/windmusic.desktop" > "${'$'}DESKTOP_DIR/com-maxrave-windmusic-MainKt.desktop"
        ...
        // Conveyor's launcher lives at output/bin/windmusic (lowercase).
        val appExecutable = appDir.resolve("bin/windmusic")
...
tasks.register("buildLinuxAppImage") {
    group = "distribution"
    description = "Full WindMusic Desktop Linux AppImage build pipeline (vlcSetup → conveyor → AppImage)."
...
tasks.register("buildMacZipAmd64") {
    group = "distribution"
    description = "Full WindMusic Desktop macOS Intel .zip pipeline (vlcSetup → conveyor)."
    dependsOn(conveyorMakeMacZipAmd64)
}

tasks.register("buildMacZipAarch64") {
    group = "distribution"
    description = "Full WindMusic Desktop macOS Apple Silicon .zip pipeline (vlcSetup → conveyor)."
...
tasks.register("buildWindowsMsix") {
    group = "distribution"
    description = "Full WindMusic Desktop Windows .msix pipeline (vlcSetup → conveyor)."
